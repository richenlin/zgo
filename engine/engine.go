...
// Engine is the core component of Kratos. It has two attributes.
// PluginList is an array of plugin. Adapter is the adapter of
// web framework context and Kratos context. The relationship of adapter and
...
func (eng *Engine) setConfig(cfg config.Config) *Engine {
	...
	if !sysCheck {
		panic(fmt.Sprintf("wrong Kratos version, theme %s required Kratos version are %s",
			eng.config.Theme, strings.Join(template.Default().GetRequirements(), ",")))
	}
	if !themeCheck {
		panic(fmt.Sprintf("wrong Theme version, Kratos %s required Theme version are %s",
